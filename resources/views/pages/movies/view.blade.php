<x-app-layout>
    @php
        ...
        $trailerRaw = trim((string) ($movie->trailer_url ?? ''));
        ...
        $movieGenresText = $movie->genres->pluck('name')->implode(', ') ?: 'N/A';
        $metaDescription = \Illuminate\Support\Str::limit(strip_tags($movie->description), 160);
        $metaImage = $movie->poster_image ? asset("storage/{$movie->poster_image}") : asset('images/logo.png');
    @endphp

    @section('meta_title', $movie->title . ' - CG Chartbusters')
            ...
            <section class="order-1 lg:col-span-7">
                <div class="hero-media w-full overflow-hidden rounded-xl bg-black shadow-2xl aspect-video border border-gray-700/30">
                    @if($trailerEmbedIframeSrc)
                        <iframe src="{{ $trailerEmbedIframeSrc }}" title="{{ $movie->title }} trailer" class="w-full h-full" loading="lazy"
                    ...
                    @elseif($trailerIframeUrl)
                        <iframe src="{{ $trailerIframeUrl }}" title="{{ $movie->title }} trailer" class="w-full h-full" loading="lazy"
                            allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture"
                            allowfullscreen referrerpolicy="strict-origin-when-cross-origin"></iframe>
                    @elseif($trailerVideoUrl)
                        <video src="{{ $trailerVideoUrl }}" class="w-full h-full" controls playsinline preload="metadata"></video>
                    @elseif($movie->poster_image_landscape)
                        <img src="{{ asset("storage/{$movie->poster_image_landscape}") }}" alt="{{ $movie->title }}"
                ...
                <div class="space-y-4">
                    <div class="flex flex-col sm:flex-row items-center sm:items-start gap-4 text-center sm:text-left">
                        ...
                        <div class="min-w-0 flex-1">
                            <h1 class="break-words text-xl sm:text-2xl font-black text-white leading-tight">{{ $movie->title }}</h1>
                            <div class="mt-2 inline-flex items-center gap-1 text-xs text-yellow-500 font-bold bg-gray-900/60 px-2.5 py-1 rounded-full border border-gray-700/30 justify-center sm:justify-start">
                                ...
                                <span>{{ $movie->cg_chartbusters_ratings ?? 0 }}/10</span>
                                <span class="text-gray-400 font-medium">({{ $reviews->total() }} Votes)</span>
                            </div>
                            <div class="mt-3 space-y-1 text-xs sm:text-sm text-gray-300">
                                <p><span class="text-gray-400 font-medium">Released:</span> {{ $movie->release_date ? \Carbon\Carbon::parse($movie->release_date)->format($movie->is_release_year_only ? 'Y' : 'd M Y') : 'N/A' }}</p>
            ...
            <section class="order-3 lg:col-span-12 rounded-xl border border-gray-700/50 bg-gray-800/40 p-4 sm:p-6 shadow-xl">
                <div class="flex flex-col sm:flex-row sm:items-center justify-between mb-4 gap-4 pb-3 border-b border-gray-700/30">
                    <h3 class="text-lg font-black text-white flex items-center gap-2">
                        <span class="w-1 h-5 bg-yellow-500 rounded"></span> {{ $movie->title }} Plot
                    </h3>
