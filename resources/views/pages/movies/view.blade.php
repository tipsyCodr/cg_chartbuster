            ...
            <div class="lg:col-span-2 hero-section p-4">
                <div class="hero-media h-full w-full">
                    ...
                    @elseif($movie->poster_image_landscape)
                        <img src="{{ asset("storage/{$movie->poster_image_landscape}") }}" alt="{{ $movie->title }}" class="w-full h-full object-cover rounded-lg">
                    @elseif($movie->poster_image)
                        <img src="{{ asset("storage/{$movie->poster_image}") }}" alt="{{ $movie->title }}" class="w-full h-full object-cover rounded-lg">
                    @endif
                </div>
            </div>
            ...
            <div class="lg:col-span-1 p-6 flex flex-col justify-between">
                <div>
                    ...
                    <div class="text-gray-300 mb-2"><strong>Released:</strong> {{ date('Y', strtotime($movie->release_date)) }}</div>
                     @php
                    $genre = \App\Models\Genre::find((int)$movie->genre);
                    @endphp
