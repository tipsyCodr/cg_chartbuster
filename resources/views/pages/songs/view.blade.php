        ...
        <div class="grid gap-6 lg:grid-cols-12">
            ...
            <div class="order-2 space-y-6 lg:order-2 lg:col-span-6">
                    ...
                    <div class="space-y-4">
                        <div class="flex items-start gap-4">
                            ...
                            <div class="min-w-0 flex-1">
                                <h1 class="break-words text-2xl font-bold text-white md:text-3xl">{{ $song->title }}</h1>
                                <div class="mt-2 inline-flex items-center gap-1 text-gray-200">
                                    <i class="fa-solid fa-star text-yellow-400"></i>
                                    <span class="font-semibold">{{ $song->cg_chartbusters_ratings ?? 0 }}/10</span>
                                    <span>({{ $reviews->total() }} Votes)</span>
                                </div>
                                <div class="mt-3 space-y-1 text-sm text-gray-300">
                                    <p>Released: {{ $song->release_date ? \Carbon\Carbon::parse($song->release_date)->format($song->is_release_year_only ? 'Y' : 'd M Y') : 'N/A' }}</p>
                                    <p>Genres: {{ $songGenresText }}</p>
                                    <p>Language: {{ $song->region?->name ?? 'N/A' }}</p>
                                    <p>CBFC: {{ $song->cbfc ?: 'NA' }}</p>
                                </div>
                                <div class="mt-4">
                ...
                <section class="rounded-lg border border-gray-700 bg-gray-800/30 p-4 sm:p-5">
                    <h3 class="mb-3 text-xl font-semibold text-white">{{ $song->title }} Lyrics:</h3>
