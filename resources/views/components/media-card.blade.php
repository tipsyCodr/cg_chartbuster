...
<div {{ $attributes->merge(['class' => 'group/card relative bg-gray-900/50 rounded-xl border border-white/5 hover:border-yellow-500/30 transition-all duration-300 flex flex-col']) }}>
    ...
    @if(isset($index))
        <span class="absolute -left-8 md:-left-12 bottom-4 z-30 font-black text-yellow-500 text-8xl md:text-9xl not-italic select-none drop-shadow-[4px_0_8px_rgba(0,0,0,0.6)] transform group-hover/card:scale-110 transition-transform pointer-events-none">
            {{ $index }}
        </span>
    @endif
