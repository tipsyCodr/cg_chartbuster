@props([
    'href',
    'active' => false,
    'icon' => null,
])

<li>
    <a href="{{ $href }}" @class([
        'group flex items-center px-4 py-3 text-sm font-bold transition-all duration-200 rounded-2xl',
        'bg-blue-600 text-white shadow-lg shadow-blue-900/40 active-nav' => $active,
        'text-slate-400 hover:bg-slate-800 hover:text-white' => !$active,
    ])>
        @if($icon)
            <div @class([
                'mr-3 p-2 rounded-xl transition-all duration-200',
                'bg-blue-500 text-white shadow-lg shadow-blue-900/20' => $active,
                'bg-slate-800 text-slate-500 group-hover:bg-slate-700 group-hover:text-blue-400 transition-all' => !$active,
            ])>
                <i class="{{ $icon }} text-base"></i>
            </div>
        @endif
        <span class="flex-1">{{ $slot }}</span>
        
        @if($active)
            <div class="w-1.5 h-1.5 rounded-full bg-white ml-2"></div>
        @endif
    </a>
</li>
