<x-admin-layout title="Song Submissions">
    <div class="space-y-6">
        <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            <div>
                <h2 class="text-2xl font-bold text-gray-800 dark:text-white">Song Submissions</h2>
                <p class="text-sm text-gray-600 dark:text-gray-400">Review and moderate user-submitted songs.</p>
            </div>
            <form action="{{ route('admin.song-submissions.index') }}" method="GET">
        ...
        <div class="bg-white dark:bg-gray-800 shadow-md rounded-xl overflow-hidden border border-gray-200 dark:border-gray-700">
            <table class="w-full text-sm text-left text-gray-500">
                <thead class="text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 border-b border-gray-200">
                    <tr>
                    ...
                    @forelse($submissions as $submission)
                        ...
                        <tr class="bg-white border-b hover:bg-gray-50 transition-colors">
                            ...
                            <td class="px-6 py-4 text-right">
                                <a href="{{ route('admin.song-submissions.show', $submission) }}" class="text-blue-600 hover:text-blue-700 font-bold text-xs uppercase">Review</a>
                            </td>
                        </tr>
                    @empty
                        <tr><td colspan="6" class="px-6 py-8 text-center text-gray-500">No song submissions found.</td></tr>
                    @endforelse
                </tbody>
            </table>
            @if($submissions->hasPages())
                <div class="px-6 py-4 border-t border-gray-200">{{ $submissions->links() }}</div>
            @endif
        </div>
    </div>
</x-admin-layout>
