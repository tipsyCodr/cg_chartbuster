<x-admin-layout title="Artist Submissions">
    <div class="space-y-6">
        <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            <div>
                <h2 class="text-2xl font-bold text-gray-800 dark:text-white">Artist Submissions</h2>
                <p class="text-sm text-gray-600 dark:text-gray-400">Review and moderate user-submitted artists.</p>
            </div>
            <form action="{{ route('admin.artist-submissions.index') }}" method="GET">
        ...
        <div class="bg-white shadow-md rounded-xl overflow-hidden border border-gray-200">
            <table class="w-full text-sm text-left text-gray-500">
                <thead class="text-xs text-gray-700 uppercase bg-gray-50 border-b border-gray-200">
                    <tr>
                        <th class="px-6 py-3">#</th><th class="px-6 py-3">Name</th><th class="px-6 py-3">Submitted By</th>
                    ...
                    @forelse($submissions as $submission)
                        ...
                        <tr class="bg-white border-b hover:bg-gray-50 transition-colors">
                            ...
                            <td class="px-6 py-4 font-medium text-gray-900 flex items-center gap-3">
                                @if($submission->photo)
                                    <img src="{{ asset('storage/' . $submission->photo) }}" class="w-8 h-8 rounded-full object-cover border border-gray-200">
                                @endif
                                {{ $submission->name }}
                            </td>
                            ...
                            <td class="px-6 py-4 text-right"><a href="{{ route('admin.artist-submissions.show', $submission) }}" class="text-blue-600 hover:text-blue-700 font-bold text-xs uppercase">Review</a></td>
                        </tr>
                    @empty
                        <tr><td colspan="6" class="px-6 py-8 text-center text-gray-500">No artist submissions found.</td></tr>
                    @endforelse
                </tbody>
            </table>
