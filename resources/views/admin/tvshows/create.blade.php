...
@section('page-title', 'Add New TV Show')

@section('content')
<div class="max-w-5xl mx-auto space-y-8">
    <div class="flex items-center justify-between">
        <div>
            <h2 class="text-2xl font-black text-gray-800 tracking-tight">ADD NEW TV SHOW</h2>
            <p class="text-xs text-gray-400 font-bold uppercase tracking-widest mt-1">Create a new series entry in the database</p>
        </div>
    ...
    <form action="{{ route('admin.tvshows.store') }}" method="POST" enctype="multipart/form-data" class="space-y-8">
        @csrf
        ...
        <x-admin.form-section title="Basic Information" description="Show title, plot and release details">
            <x-admin.form-group label="Show Title" for="title" required :error="$errors->first('title')">
                <x-admin.input type="text" name="title" id="title" required value="{{ old('title') }}" placeholder="Enter TV show title" :error="$errors->has('title')" />
            </x-admin.form-group>
        ...
        <x-admin.form-section title="Media Assets" description="Posters and promotional content">
            ...
            <div class="md:col-span-2">
                <x-admin.form-group label="Trailer / Promo URL" help="Link to the show's teaser or trailer">
                    <x-admin.input type="url" name="trailer_url" value="{{ old('trailer_url') }}" placeholder="https://..." />
                </x-admin.form-group>
            </div>
        </x-admin.form-section>
        ...
        <x-admin.form-section title="Promotions" description="Banner display and links">
            ...
            <x-admin.form-group label="Banner Label">
                <x-admin.input type="text" name="banner_label" value="{{ old('banner_label') }}" placeholder="e.g. Streaming Now" />
            </x-admin.form-group>
