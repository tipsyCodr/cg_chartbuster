                        ...
                        <div class="">
                            ...
                            <div class="mb-4">
                                ...
                                <select name="banner_label" id="banner_label" class="mt-1 block w-full rounded-md border p-2 border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50">
                                    <option value="" {{ empty($songs->banner_label) ? 'selected' : '' }}>None</option>
                                    <option value="🎬 Watch on Official YouTube Channel" {{ $songs->banner_label == '🎬 Watch on Official YouTube Channel' ? 'selected' : '' }}>🎬 Watch on Official YouTube Channel</option>
                                    <option value="🎥 Watch in Theaters" {{ $songs->banner_label == '🎥 Watch in Theaters' ? 'selected' : '' }}>🎥 Watch in Theaters</option>
                                    <option value="🔜 Upcoming Release" {{ $songs->banner_label == '🔜 Upcoming Release' ? 'selected' : '' }}>🔜 Upcoming Release</option>
                                </select>
                            </div>

                            <div class="mb-4">
                                <label for="banner_link" class="block my-1 text-sm font-medium text-gray-700">Banner Link (Url)</label>
                                <input type="url" name="banner_link" id="banner_link" value="{{ $songs->banner_link }}"
                                ...
                                <div>
                                    <label for="content_description_chh"
                                        class="block my-1 text-sm font-medium text-gray-700">Lyrics (Chhattisgarhi)</label>
                                    <textarea name="content_description_chh" id="content_description_chh" rows="3" placeholder="छत्तीसगढ़ी मं गीत के बोल एहाँ भरव…"
                                        class="mt-1 block w-full @error('content_description_chh') is-invalid @enderror rounded-md  border p-2 border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50">{{ $songs->content_description_chh }}</textarea>
                                ...
                                <div>
                                    <label for="release_date" class="block my-1 text-sm font-medium text-gray-700">Release
                                        Date</label>
                                    <input type="date" name="release_date" id="release_date" value="{{ $songs->release_date?->format('Y-m-d') }}"
                                        class="w-full p-2 my-2 border border-gray-300 rounded">
                                    <div class="flex items-center">
                                        <input type="hidden" name="is_release_year_only" value="0">
                                        <input type="checkbox" name="is_release_year_only" id="is_release_year_only" value="1" {{ old('is_release_year_only', $songs->is_release_year_only) ? 'checked' : '' }} class="w-4 h-4 text-yellow-600 border-gray-300 rounded focus:ring-yellow-500">
                        ...
                        dateInput.value = dateInput.value + '-01-01';
                                ...
                                <div>
                                    ...
                                    <x-searchable-multiselect 
                                        :options="$genres" 
                                        :selected="old('genre_ids', $songs->genres->pluck('id')->toArray())" 
                                        name="genre_ids" 
                                ...
                                <div x-data="{ hours: 0, minutes: 0 }">
                                 ...
                                 {{ !empty($songs->artists) ? json_encode($songs->artists->flatMap(function($artist) {
                                ...
                                <div x-data="{ 
                                     artistEntries: {{ !empty($songs->artists) ? json_encode($songs->artists->flatMap(function($artist) {
                                        $roles = $artist->pivot->artist_category_ids ?? [];
                                        if (empty($roles)) return [['artist' => $artist->id, 'role' => '']];
                                        return collect($roles)->map(function($roleId) use ($artist) {
                                            return [
