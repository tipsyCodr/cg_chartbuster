...
@section('page-title', 'Edit Song: ' . $song->title)

@section('content')
<div class="max-w-5xl mx-auto space-y-8">
    <div class="flex items-center justify-between">
        <div>
            <h2 class="text-2xl font-black text-gray-800 tracking-tight uppercase">EDIT: {{ $song->title }}</h2>
            <p class="text-xs text-gray-400 font-bold uppercase tracking-widest mt-1">Update song details and credits</p>
        </div>
    ...
    <form action="{{ route('admin.songs.update', $song) }}" method="POST" enctype="multipart/form-data" class="space-y-8">
        @csrf
        @method('PUT')
        ...
        <x-admin.form-section title="Basic Information" description="Song title, album and release details">
            <x-admin.form-group label="Song Title" for="title" required :error="$errors->first('title')">
                <x-admin.input type="text" name="title" id="title" required value="{{ old('title', $song->title) }}" placeholder="Enter song title" :error="$errors->has('title')" />
            </x-admin.form-group>

            <x-admin.form-group label="Album Name" for="album">
                <x-admin.input type="text" name="album" id="album" value="{{ old('album', $song->album) }}" placeholder="e.g. Laila Majnu" />
            </x-admin.form-group>

            <x-admin.form-group label="Release Date" for="release_date" :error="$errors->first('release_date')">
                <x-admin.release-date
                    :value="old('release_date', $song->release_date?->format('Y-m-d'))"
                    :year-only-value="old('is_release_year_only', $song->is_release_year_only)"
                    :error="$errors->first('release_date')"
                />
            </x-admin.form-group>

            <x-admin.form-group label="Duration" for="duration" :error="$errors->first('duration')">
                <x-admin.input type="time" name="duration" id="duration" value="{{ old('duration', $song->duration ? substr((string) $song->duration, 0, 5) : '') }}" :error="$errors->has('duration')" />
            </x-admin.form-group>

            <div class="md:col-span-2">
                <x-admin.form-group label="Lyrics Snippet / Description" for="description" :error="$errors->first('description')">
                    <x-admin.textarea name="description" id="description" rows="3">{{ old('description', $song->description) }}</x-admin.textarea>
                </x-admin.form-group>
            </div>
        ...
        <x-admin.form-section title="Media Assets" description="Posters and video links">
            <x-admin.form-group label="Song Poster (Portrait)">
                <x-admin.file-upload name="poster_image" label="Portrait Poster" :current="$song->poster_image" />
            </x-admin.form-group>

            <x-admin.form-group label="Song Poster (Landscape)">
                <x-admin.file-upload name="poster_image_landscape" label="Landscape Poster" :current="$song->poster_image_landscape" />
            </x-admin.form-group>

            <div class="md:col-span-2">
                <x-admin.form-group label="Video Link (YouTube)" for="trailer_url" help="Link to the music video or audio stream">
                    <x-admin.input type="url" name="trailer_url" id="trailer_url" value="{{ old('trailer_url', $song->trailer_url) }}" placeholder="https://youtube.com/watch?v=..." />
                </x-admin.form-group>
            </div>
        </x-admin.form-section>
        ...
        <x-admin.form-section title="Credits & Metadata" description="Featured artists and song classification">
            <x-admin.form-group label="Genres" required>
                <div class="mt-1">
                    <x-searchable-multiselect 
                        :options="$genres" 
                        :selected="old('genre_ids', $song->genres->pluck('id')->toArray())" 
                        name="genre_ids" 
            ...
            <x-admin.form-group label="Language / Region" for="region_id">
                <x-admin.select name="region_id">
                    ...
                    @foreach($regions as $region)
                        <option value="{{ $region->id }}" {{ old('region_id', $song->region_id) == $region->id ? 'selected' : '' }}>{{ $region->name }}</option>
                    @endforeach
                </x-admin.select>
            </x-admin.form-group>
            ...
            <x-admin.form-group label="Production House" for="production_house_id" :error="$errors->first('production_house_id')">
                <x-admin.select name="production_house_id" id="production_house_id" :error="$errors->has('production_house_id')">
                    ...
                    @foreach($productionHouses as $ph)
                        <option value="{{ $ph->id }}" {{ old('production_house_id', $song->production_house_id) == $ph->id ? 'selected' : '' }}>{{ $ph->name }}</option>
                    @endforeach
                </x-admin.select>
            </x-admin.form-group>
            ...
            <x-admin.form-group label="CG Chartbusters Rating">
                <div class="mt-1 bg-gray-50/50 p-3 rounded-xl border border-gray-100 shadow-inner">
                    <x-star-rating id="rating" name="cg_chartbusters_ratings" :value="old('cg_chartbusters_ratings', $song->cg_chartbusters_ratings ?? 0)"></x-star-rating>
                </div>
            </x-admin.form-group>
            ...
            <div class="md:col-span-2">
                <x-admin.form-group label="Artists & Credits" help="Assign singers, composers, and writers">
                    @php
                        $currentArtists = $song->artists->flatMap(function($artist) {
        ...
        <x-admin.form-section id="promotions" title="Promotions" description="Banner display and links">
             <x-admin.form-group label="Show on Banner">
                <x-admin.select name="show_on_banner">
                    <option value="0" {{ old('show_on_banner', $song->show_on_banner) == 0 ? 'selected' : '' }}>No</option>
                    <option value="1" {{ old('show_on_banner', $song->show_on_banner) == 1 ? 'selected' : '' }}>Yes</option>
                </x-admin.select>
            </x-admin.form-group>

            <x-admin.form-group label="Banner Label">
                <x-admin.input type="text" name="banner_label" value="{{ old('banner_label', $song->banner_label) }}" placeholder="e.g. New Single Out Now!" />
            </x-admin.form-group>

            <div class="md:col-span-2">
                <x-admin.form-group label="Action Link">
                    <x-admin.input type="url" name="banner_link" value="{{ old('banner_link', $song->banner_link) }}" placeholder="https://..." />
                </x-admin.form-group>
            </div>
        </x-admin.form-section>
        ...
        <div class="sticky bottom-8 z-30 p-4 bg-white/80 backdrop-blur-md rounded-2xl border border-gray-100 shadow-2xl flex items-center justify-between">
             <div class="flex items-center gap-2 text-[10px] font-black text-gray-400 uppercase tracking-widest pl-2">
                ...
                Last updated {{ $song->updated_at->diffForHumans() }}
            </div>
