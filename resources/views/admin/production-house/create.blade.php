...
@section('page-title', 'Add New Production House')

@section('content')
<div class="max-w-5xl mx-auto space-y-8">
    <div class="flex items-center justify-between">
        <div>
            <h2 class="text-2xl font-black text-gray-800 tracking-tight">ADD NEW PRODUCTION HOUSE</h2>
            <p class="text-xs text-gray-400 font-bold uppercase tracking-widest mt-1">Create a new profile for a production house or studio</p>
        </div>
    ...
    <form action="{{ route('admin.productionhouses.store') }}" method="POST" enctype="multipart/form-data" class="space-y-8">
        @csrf
        ...
        <x-admin.form-section title="Basic Information" description="Production House name, founding details and location">
            <x-admin.form-group label="Production House Name" for="name" required :error="$errors->first('name')">
                <x-admin.input type="text" name="name" id="name" required value="{{ old('name') }}" placeholder="e.g. CGI Chartbusters Studios" :error="$errors->has('name')" />
            </x-admin.form-group>
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6 md:col-span-2">
                <x-admin.form-group label="Founded Year" for="founded_year" :error="$errors->first('founded_year')">
                    <x-admin.input type="number" name="founded_year" id="founded_year" value="{{ old('founded_year') }}" placeholder="e.g. 2015" />
                </x-admin.form-group>

                <x-admin.form-group label="Owner / Founder Name" for="owner_name" :error="$errors->first('owner_name')">
                    <x-admin.input type="text" name="owner_name" id="owner_name" value="{{ old('owner_name') }}" placeholder="e.g. John Doe" />
                </x-admin.form-group>

                <x-admin.form-group label="Active Since" for="active_since" :error="$errors->first('active_since')">
                    <x-admin.input type="number" name="active_since" id="active_since" value="{{ old('active_since') }}" placeholder="e.g. 2015" />
                </x-admin.form-group>
            </div>
        ...
        <x-admin.form-section title="Media & Visual Assets" description="Upload studio logos and landscape cover banners">
            <x-admin.form-group label="Logo / Profile Photo" help="High quality square image/logo" required :error="$errors->first('photo')">
                <x-admin.file-upload name="photo" label="Logo Image" />
            </x-admin.form-group>
        ...
        <x-admin.form-section title="Social Presence & Credentials" description="Studio websites, channels, and rating configurations">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 md:col-span-2">
                <x-admin.form-group label="Official Website URL" for="website_url" :error="$errors->first('website_url')">
                    <x-admin.input type="text" name="website_url" id="website_url" value="{{ old('website_url') }}" placeholder="https://example.com" />
                </x-admin.form-group>

                <x-admin.form-group label="YouTube Channel URL" for="youtube_url" :error="$errors->first('youtube_url')">
                    <x-admin.input type="text" name="youtube_url" id="youtube_url" value="{{ old('youtube_url') }}" placeholder="https://youtube.com/..." />
                </x-admin.form-group>

                <x-admin.form-group label="Instagram Profile URL" for="instagram_url" :error="$errors->first('instagram_url')">
                    <x-admin.input type="text" name="instagram_url" id="instagram_url" value="{{ old('instagram_url') }}" placeholder="https://instagram.com/..." />
                </x-admin.form-group>

                <x-admin.form-group label="Facebook Page URL" for="facebook_url" :error="$errors->first('facebook_url')">
                    <x-admin.input type="text" name="facebook_url" id="facebook_url" value="{{ old('facebook_url') }}" placeholder="https://facebook.com/..." />
                </x-admin.form-group>

                <x-admin.form-group label="Twitter/X Profile URL" for="twitter_url" :error="$errors->first('twitter_url')">
                    <x-admin.input type="text" name="twitter_url" id="twitter_url" value="{{ old('twitter_url') }}" placeholder="https://twitter.com/..." />
                </x-admin.form-group>
            ...
            <div class="grid grid-cols-2 gap-6 md:col-span-2 mt-4">
                <label class="flex items-center gap-3 p-3 rounded-xl border border-gray-100 bg-gray-50/30 hover:bg-white hover:shadow-sm transition-all cursor-pointer group">
                    <input type="checkbox" name="is_featured" value="1" {{ old('is_featured') ? 'checked' : '' }} class="w-4 h-4 rounded border-gray-200 text-blue-600 focus:ring-blue-500/20">
                ...
                <label class="flex items-center gap-3 p-3 rounded-xl border border-gray-100 bg-gray-50/30 hover:bg-white hover:shadow-sm transition-all cursor-pointer group">
                    <input type="checkbox" name="is_verified" value="1" {{ old('is_verified') ? 'checked' : '' }} class="w-4 h-4 rounded border-gray-200 text-blue-600 focus:ring-blue-500/20">
                    <div class="flex flex-col">
