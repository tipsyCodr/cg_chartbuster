...
<div class="max-w-5xl mx-auto space-y-8" x-data="{ isProductionHouse: false }" x-init="isProductionHouse = Array.from($el.querySelectorAll('input[data-name=\'Production House\']')).some(el => el.checked)">
    <div class="flex items-center justify-between">
        <div>
            <h2 class="text-2xl font-black text-gray-800 tracking-tight" x-text="isProductionHouse ? 'ADD NEW PRODUCTION HOUSE' : 'ADD NEW ARTIST'">ADD NEW ARTIST</h2>
            <p class="text-xs text-gray-400 font-bold uppercase tracking-widest mt-1" x-text="isProductionHouse ? 'Create a new profile for a production house' : 'Create a new profile for an artist'">Create a new profile for an artist</p>
        </div>
    ...
    <form action="{{ route('admin.artists.store') }}" method="POST" enctype="multipart/form-data" class="space-y-8">
        ...
        <div class="sticky bottom-8 z-30 p-4 bg-white/80 backdrop-blur-md rounded-2xl border border-gray-100 shadow-2xl flex items-center justify-end gap-3">
            ...
            <button type="submit" class="px-10 py-3 bg-blue-600 text-white rounded-xl text-xs font-black uppercase tracking-widest shadow-xl shadow-blue-200 hover:bg-blue-700 hover:shadow-blue-300 transition-all transform active:scale-95" x-text="isProductionHouse ? 'Save Production House' : 'Save Artist Profile'">
                Save Artist Profile
            </button>
        </div>
    </form>
