    ...
    <form method="POST" action="{{ route('register') }}" class='border px-20 py-8 border-b-yellow-400 border-b-8 rounded-t-lg'>
        ...
        <div class="block mt-4">
            <label for="terms" class="inline-flex items-center">
                <input id="terms" type="checkbox" class="rounded border-gray-300 text-yellow-500 shadow-sm focus:ring-yellow-500" name="terms" required>
                <span class="ms-2 text-sm text-gray-400">
                    {{ __('I agree to the') }} 
                    <a href="{{ route('terms-and-conditions') }}" class="underline text-yellow-500 hover:text-yellow-400" target="_blank">{{ __('Terms and Conditions') }}</a>
                </span>
            </label>
